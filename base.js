...
const MIRRORED = { // mirrored pieces
	Z: 'S',
	L: 'J',
	O: 'O',
	S: 'Z',
...
$('#b').height = (boardSize[1] - hiddenRows + 2) * cellSize;
$('#b').width = boardSize[0] * cellSize;
...
$('#b').onmousemove = function mousemove(e) {
...
$('#b').onmousedown = function mousedown(e) {
	rect = $('#b').getBoundingClientRect();
...
document.onmouseup = function mouseup() {
	...
	if (drawMode) {
		...
		if (drawn.length == 4 && $('#autocolor').checked) {
...
if (/(tablet|ipad|playbook|silk)|(android(?!.*mobi))/i.test(ua)) {
	...
	$('#tcc').style.display = 'inline-block';
} else if (
...
function mirror() {
	for (row = 0; row < board.length; row++) {
		...
		for (i = 0; i < board[row].length; i++) {
			if (board[row][i].t == 1) board[row][i].c = MIRRORED[board[row][i].c];
		}
	}
	for (i = 0; i < queue.length; i++) {
		queue[i] = MIRRORED[queue[i]];
	}
	holdP = MIRRORED[holdP];
	piece = MIRRORED[piece];
	...
	for (i = 0; i < hist.length; i++) {
		...
		for (row = 0; row < tempBoard.length; row++) {
			...
			for (j = 0; j < tempBoard[row].length; j++) {
				if (tempBoard[row][j].t == 1) tempBoard[row][j].c = MIRRORED[tempBoard[row][j].c];
			}
		}
		hist[i]['board'] = JSON.stringify(tempBoard);
		tempQueue = JSON.parse(hist[i]['queue']);
		for (j = 0; j < tempQueue.length; j++) {
			tempQueue[j] = MIRRORED[tempQueue[j]];
		}
		hist[i]['queue'] = JSON.stringify(tempQueue);

		hist[i]['hold'] = MIRRORED[hist[i]['hold']];
		hist[i]['piece'] = MIRRORED[hist[i]['piece']];
	}
	board = tempBoard;
	queue = tempQueue;
	holdP = MIRRORED[holdP];
...
function callback() {
	...
	$('#tc-re').addEventListener('touchstart', function (e) {
		input = 'RE';
		keysDown |= flags[input];
		restart();
	});

	$('#tc-hd').addEventListener('touchstart', function (e) {
		input = 'HD';
		keysDown |= flags[input];
		hardDrop();
	});

	$('#tc-h').addEventListener('touchstart', function (e) {
		input = 'HL';
		keysDown |= flags[input];
		hold();
	});

	$('#tc-dr').addEventListener('touchstart', function (e) {
		input = 'R180';
		keysDown |= flags[input];
		rotate('R180');
	});

	$('#tc-cc').addEventListener('touchstart', function (e) {
		input = 'CCW';
		keysDown |= flags[input];
		rotate('CCW');
	});

	$('#tc-c').addEventListener('touchstart', function (e) {
		input = 'CW';
		keysDown |= flags[input];
		rotate('CW');
	});

	$('#tc-d').addEventListener('touchstart', function (e) {
		input = 'SD';
		keysDown |= flags[input];
		sdID++;
		softDrop(sdID);
	...
	$('#tc-d').addEventListener('touchend', function (e) {
		input = 'SD';
		if (keysDown & flags[input]) keysDown ^= flags[input];
		sdID++;
	});

	$('#tc-r').addEventListener('touchstart', function (e) {
		input = 'R';
		keysDown |= flags[input];
	});

	$('#tc-r').addEventListener('touchend', function (e) {
		input = 'R';
		if (keysDown & flags[input]) keysDown ^= flags[input];
	...
	$('#tc-l').addEventListener('touchstart', function (e) {
		input = 'L';
		keysDown |= flags[input];
	});

	$('#tc-l').addEventListener('touchend', function (e) {
		input = 'L';
		if (keysDown & flags[input]) keysDown ^= flags[input];
		if (!(keysDown & flags.L) && !(keysDown & flags.R)) {
			dasID++;
